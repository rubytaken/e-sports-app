...
export function StatCard({ label, value, icon, accent, className }: StatCardProps) {
  return (
    ...
    >
      ...
      <p className={cn("text-2xl font-extrabold tracking-tight", accent ? "text-accent" : "text-text-0")}>
        {value}
      </p>
    </div>
  );
      ...
      <div className="flex items-center gap-1 mt-3">
        {results.map((r, i) => (
          <div
            key={i}
            className={cn(
              "h-7 w-7 rounded-lg flex items-center justify-center text-[10px] font-bold ring-1",
              r === "W"
